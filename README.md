<div align="center"><center>

## Archival Notice

As of v1.42.0, extended zoom-out is a built-in option in Xaero's World Map, so this mod is no
longer required.

<img alt="Xaero's World Map Settings" width=500 src="https://raw.githubusercontent.com/TerminalMC/XaeroZoomout/HEAD/assets/images/xaerosworldmap_01.png">

---

<img alt="Icon" width=100 src="https://raw.githubusercontent.com/TerminalMC/XaeroZoomout/HEAD/common/src/main/resources/assets/xaerozoomout/icon.png">

## XaeroZoomout

Reduces the minimum zoom of [Xaero's World Map](https://modrinth.com/project/NcUtCpym) from 0.0625x to 0.0025x.

[![Environment](https://img.shields.io/badge/Environment-Client-blue?logo=data:image/png;base64,)]()
[![Latest Minecraft](https://img.shields.io/modrinth/game-versions/T6oqPfxF?label=Latest%20Minecraft&color=%2300AF5C&logo=data:image/png;base64,)](https://modrinth.com/project/T6oqPfxF/versions)

[![Loader](https://img.shields.io/badge/Available%20for-Fabric-dbd0b4?logo=data:image/png;base64,)](https://fabricmc.net/)
[![Loader](https://img.shields.io/badge/Available%20for-NeoForge-f16436?logo=data:image/png;base64,)](https://neoforged.net/)

[![Available on Modrinth](https://img.shields.io/modrinth/dt/T6oqPfxF?label=Available%20on%20Modrinth&logo=modrinth&logoColor=%2300AF5C)](https://modrinth.com/project/T6oqPfxF)
[![Available on CurseForge](https://img.shields.io/curseforge/dt/845615?label=Available%20on%20CurseForge&logo=curseforge)](https://curseforge.com/minecraft/mc-mods/xaerozoomout)
[![Available on GitHub](https://img.shields.io/github/downloads/TerminalMC/XaeroZoomout/total?label=Available%20on%20GitHub&logo=github&logoColor=white)](https://github.com/TerminalMC/XaeroZoomout)

</center></div>

### Features

No settings, no setup; just zoom out and watch it work.

As a guide, the Xaero's World Map display is typically about 30k blocks wide when
fully zoomed out. With this mod, the display width is increased to about 760k.

Note: Rendering the world map is quite CPU-intensive, so you'll probably experience
some lag when fully zoomed out.

### Contact

[![Discord Server](https://img.shields.io/discord/1103153365216669797?logo=discord&label=Discord%20Server&color=%235865F2)](https://discord.terminalmc.dev)

[![GitHub Issues](https://img.shields.io/github/issues/TerminalMC/XaeroZoomout?logo=github&label=GitHub%20Issues)](https://github.com/TerminalMC/XaeroZoomout/issues)

[![License](https://img.shields.io/github/license/TerminalMC/XaeroZoomout?label=License&logo=github&logoColor=white)](https://github.com/TerminalMC/XaeroZoomout/blob/HEAD/LICENSE.txt)
